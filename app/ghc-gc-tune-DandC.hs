...
tuningSpace :: Options -> IO ValidOpts
tuningSpace opts = do
...
-- TODO: until max core Num
coreOptCheck :: [Int] -> Int -> [Int]
coreOptCheck xs core = if xs == [] then [1..core] else xs

getCoreOpt :: (String, [Int]) -> [Int]
getCoreOpt ([],[]) = []
getCoreOpt (xs,ys) = ys

coreParse xs = foldr f ([],[]) ('$' : xs)
    where
...
main = do
    ...
    (coreinfo, exe, args)   <- case args' of
                                []       -> ioError (userError (usageInfo header options))
                                -- TODO: can process no core option
                                (x:y:xs) -> return (x,y,xs)

    -- get core number
    -- TODO: mk core option which can use like "--coreopt=1,3,5,6"
    core' <- readProcess "grep" ["-c", "processor", "/proc/cpuinfo"] []
    let x = read core'::Int
    let newopt = "--coreopt="
...
best :: Int -> String -> Options -> String -> String -> String -> String -> (GCStats -> Double) -> [(GCHooks, Maybe GCStats)] -> IO ()
best coreN exe opts short title unit longunit field stats = do
    let best5 = take 5 $ sortBy (comparing thd3) [(sizeA gs, sizeH gs, field r) | (gs, Just r) <- stats]
    putStrLn $ "Best settings for " ++ title ++ ":"
    forM best5 $ \(bestA, bestH, bestF) -> printf "T:\t%.2f\tA:\t%d\tH:\t%d\tC:\t%d\n" bestF bestA bestH coreN
